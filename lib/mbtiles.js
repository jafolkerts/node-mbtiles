...
function MBTiles(uri, callback) {
    ...
    mbtiles._db = new sqlite3.Database(mbtiles.filename, function (err) {
        ...
        if (process.platform === "win32" && mbtiles.filename.charAt(0) === '/')
            mbtiles.filename = mbtiles.filename.slice(1);
...
MBTiles.prototype._exists = function (table, callback) {
    ...
    this._db.all(sql, function (err, rows) {
        if (err) return callback(err);
        mbtiles._schema = rows.map(function (r) {
            return r.name
        });
        ...
        zlib.inflate(!Buffer.isBuffer(row.grid) ? new Buffer(row.grid, 'binary') : row.grid, function (err, buffer) {
            ...
            mbtiles._db.all(sqljson, z, x, y, function (err, rows) {
                ...
                for (var i = 0; i < rows.length; i++) {
                    try {
                        grid.data[rows[i].key_name] = JSON.parse(rows[i].key_json);
                    }
                    catch (err) {
                        return callback(new Error('Grid is invalid:' + err.message))
                    }
...
MBTiles.prototype.geocoderMigrateDocs = function (rows, callback) {
    ...
    function tms2zxy(zxys) {
        return zxys.split(',').map(function (tms) {
            var zxy = tms.split('/').map(function (v) {
    ...
    if ('zxy' in row && text) {
        ...
        if (doc.bounds) doc._bbox = doc.bounds.split(',').map(function (v) {
            return parseFloat(v)
        });
        delete doc.score;
        delete doc.bounds;
